    ...
    <div className="space-y-6 max-w-full overflow-hidden flex flex-col h-[calc(100vh-6rem)]">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between shrink-0 gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Acompanhe o progresso do fluxo de produção.</p>
        </div>
        {isAdmin && (
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <Users className="w-4 h-4 text-muted-foreground hidden sm:block" />
            <Select value={dentist} onValueChange={setDentist}>
              <SelectTrigger className="w-full sm:w-64 bg-white border-primary/20">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between px-1">
                      ...
                      <span className="bg-background px-2 py-0.5 rounded text-xs font-bold border text-primary">
                        {cols.length}
                      </span>
                    </div>
                    <div className="flex-1 flex flex-col gap-2 min-h-[150px]">
                      {cols.map((o) => (
                        ...
                        >
                          ...
                          <p className="text-xs text-muted-foreground mt-1 truncate pl-1">
                            {o.workType}
                          </p>
                          {isAdmin && (
                            <p className="text-[10px] font-medium text-muted-foreground mt-3 pt-2 border-t truncate pl-1">
                              {o.dentistName}
                            </p>
                          )}
                        </div>
