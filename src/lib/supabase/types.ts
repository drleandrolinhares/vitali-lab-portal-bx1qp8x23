...
//     USING: ((EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::text, 'master'::text]))))) OR ((((COALESCE(current_setting('request.jwt.claims'::text, true), '{}'::text))::jsonb -> 'user_metadata'::text) ->> 'role'::text) = ANY (ARRAY['admin'::text, 'master'::text])) OR (is_current_user_active() AND ((dentist_id = auth.uid()) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['receptionist'::text, 'technical_assistant'::text, 'financial'::text, 'relationship_manager'::text]))))))))
