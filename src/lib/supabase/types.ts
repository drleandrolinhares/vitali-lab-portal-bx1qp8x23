    ...
    Tables: {
      ...
      profiles: {
        Row: {
          ...
          authorized_kanban_stages: Json | null
...
//   authorized_kanban_stages: jsonb (nullable)
// Table: recebimentos
//   id: uuid (not null, default: gen_random_uuid())
//   data_recebimento: date (not null)
//   valor_recebido: numeric (not null, default: 0)
//   numero_caso: text (nullable)
...
// Table: order_history
//   FOREIGN KEY order_history_created_by_fkey: FOREIGN KEY (created_by) REFERENCES profiles(id) ON DELETE SET NULL
...
// Table: orders
//   FOREIGN KEY orders_acabamento_id_fkey: FOREIGN KEY (acabamento_id) REFERENCES profiles(id) ON DELETE SET NULL
...
//   FOREIGN KEY orders_maquiagem_id_fkey: FOREIGN KEY (maquiagem_id) REFERENCES profiles(id) ON DELETE SET NULL
...
//     USING: ((dentist_id = auth.uid()) OR (created_by = auth.uid()) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::text, 'master'::text, 'receptionist'::text, 'technical_assistant'::text, 'financial'::text, 'relationship_manager'::text]))))))
...
// Table: recebimentos
//   Policy "Admin recebimentos all" (ALL, PERMISSIVE) roles={public}
...
//   Policy "Public read recebimentos" (SELECT, PERMISSIVE) roles={public}
//     USING: true
// Table: scan_service_blocks
...
//     USING: ((dentist_id = auth.uid()) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::text, 'master'::text, 'receptionist'::text, 'financial'::text, 'relationship_manager'::text]))))))
...
//   CREATE INDEX idx_orders_is_repetition ON public.orders USING btree (is_repetition) WHERE (is_repetition = true)
