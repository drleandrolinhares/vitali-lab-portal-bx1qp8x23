...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    let query = supabaseAdmin.from('recebimentos').select('*').order('data_recebimento', { ascending: false })
    ...
    const valor_total = recebimentos.reduce((acc, curr) => acc + Number(curr.valor_recebido || 0), 0)
