...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { 
        email, password, name, role, clinic, phone, permissions, whatsapp_group_link, 
        personal_phone, requires_password_change, assigned_dentists, can_move_kanban_cards,
        ...
        allowed_sectors, authorized_kanban_stages
    } = await req.json()
    ...
    const canCreateUsers = isMaster || (isAdmin && callerPerms?.settings?.actions?.create_users !== false)
    ...
    if (!isAdmin) {
      ...
      if (!canAddDentist) {
        throw new Error('Unauthorized: Você não tem permissão para adicionar novos dentistas/laboratórios.')
      }
    }
    ...
    if (isAdmin && !isMaster && canOnlyCreateDentists) {
      if (role !== 'dentist') {
        throw new Error('Unauthorized: Você tem permissão para criar apenas usuários do tipo Dentista.')
      }
    }
    ...
    else if (personal_phone !== undefined) phoneToUse = typeof personal_phone === 'string' && personal_phone.trim() === '' ? null : personal_phone;
    ...
    if (whatsapp_group_link) updateData.whatsapp_group_link = whatsapp_group_link === '' ? null : whatsapp_group_link
    if (phoneToUse !== null) updateData.personal_phone = phoneToUse
    if (requires_password_change !== undefined) updateData.requires_password_change = requires_password_change
    ...
    if (Object.keys(updateData).length > 0) {
      const { error: dbError } = await supabaseAdmin.from('profiles').update(updateData).eq('id', data.user.id)
      if (dbError) {
