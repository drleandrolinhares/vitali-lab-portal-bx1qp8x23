...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: { user: callerUser }, error: callerError } = await authClient.auth.getUser(token)
    
    if (callerError || !callerUser) {
      throw new Error(`Auth session missing! Invalid or expired token: ${callerError?.message || 'User not found'}`)
    }
    ...
    const canManagePermissions = isMaster || (isAdmin && callerPerms?.settings?.actions?.manage_permissions !== false)
    ...
    const { 
        userId, email, password, name, role, clinic, phone, permissions, whatsapp_group_link, 
        personal_phone, is_active, job_function, assigned_dentists, can_move_kanban_cards,
        username, rg, cpf, birth_date, cep, address, address_number, address_complement, city, state, has_access_schedule,
        commercial_agreement, closing_date, payment_due_date, clinic_contact_name, clinic_contact_role, clinic_contact_phone,
        requires_password_change, allowed_sectors, authorized_kanban_stages
    } = body
    ...
    if (targetProfileError || !targetProfile) throw new Error(`Target profile not found: ${targetProfileError?.message}`)
    ...
    const isUpdatingDentist = canAddDentist && (targetProfile.role === 'dentist' || targetProfile.role === 'laboratory' || !targetProfile.role)

    if (!isAdmin && callerUser.id !== userId && !isUpdatingDentist) {
      throw new Error('Unauthorized: You can only update your own profile')
    }

    if (!isAdmin && (role !== undefined && role !== 'dentist' && role !== 'laboratory') && callerUser.id !== userId) {
        throw new Error('Unauthorized: Only admins can change roles to non-dentist')
    }

    const { data: targetAuthUser, error: authFetchError } = await supabaseAdmin.auth.admin.getUserById(userId)
    if (authFetchError || !targetAuthUser?.user) throw new Error(`Target auth user not found: ${authFetchError?.message}`)

    let phoneToUse: string | null | undefined = undefined;
    if (phone !== undefined) phoneToUse = typeof phone === 'string' && phone.trim() === '' ? null : phone;
    if (personal_phone !== undefined) phoneToUse = typeof personal_phone === 'string' && personal_phone.trim() === '' ? null : personal_phone;
    ...
    if (whatsapp_group_link !== undefined) authPayload.user_metadata.whatsapp_group_link = whatsapp_group_link === '' ? null : whatsapp_group_link;
    ...
    if (shouldUpdateAuth) {
        ...
        const { data: updatedAuthData, error: authError } = await supabaseAdmin.auth.admin.updateUserById(userId, authPayload)
        if (authError) {
            ...
            } else if (errorMsg.toLowerCase().includes('already registered') || errorMsg.toLowerCase().includes('already exists') || errorMsg.toLowerCase().includes('duplicate key')) {
    ...
    if (whatsapp_group_link !== undefined) updateData.whatsapp_group_link = whatsapp_group_link === '' ? null : whatsapp_group_link
    if (phoneToUse !== undefined) updateData.personal_phone = phoneToUse
    if (job_function !== undefined) updateData.job_function = job_function === '' ? null : job_function
    ...
    if (address_number !== undefined) updateData.address_number = address_number === '' ? null : address_number
    if (address_complement !== undefined) updateData.address_complement = address_complement === '' ? null : address_complement
    ...
    if (closing_date !== undefined) updateData.closing_date = closing_date === '' ? null : parseInt(closing_date, 10);
    if (payment_due_date !== undefined) updateData.payment_due_date = payment_due_date === '' ? null : parseInt(payment_due_date, 10);
    if (clinic_contact_name !== undefined) updateData.clinic_contact_name = clinic_contact_name === '' ? null : clinic_contact_name;
    if (clinic_contact_role !== undefined) updateData.clinic_contact_role = clinic_contact_role === '' ? null : clinic_contact_role;
    if (clinic_contact_phone !== undefined) updateData.clinic_contact_phone = clinic_contact_phone === '' ? null : clinic_contact_phone;
    ...
    if (isAdmin || isUpdatingDentist || isMaster) {
        ...
        if (permissions !== undefined) {
            if (!canManagePermissions) {
                throw new Error('Unauthorized: Você não tem permissão para gerenciar as permissões de acesso.')
            }
            updateData.permissions = permissions
        }
        if (assigned_dentists !== undefined) updateData.assigned_dentists = assigned_dentists
        if (can_move_kanban_cards !== undefined) updateData.can_move_kanban_cards = can_move_kanban_cards
        if (allowed_sectors !== undefined) updateData.allowed_sectors = allowed_sectors
        if (authorized_kanban_stages !== undefined) updateData.authorized_kanban_stages = authorized_kanban_stages
    }

    if (Object.keys(updateData).length > 0) {
        const { error: dbProfileError } = await supabaseAdmin.from('profiles').update(updateData).eq('id', userId)
        if (dbProfileError) {
            console.error('Profile update error:', dbProfileError)
            let errorMsg = dbProfileError.message;
            if (dbProfileError.code === '23505') { 
                if (dbProfileError.message.includes('cpf')) errorMsg = 'O CPF informado já está cadastrado em outro perfil.';
                else if (dbProfileError.message.includes('rg')) errorMsg = 'O RG informado já está cadastrado em outro perfil.';
    ...
    return new Response(JSON.stringify({ data: authData, success: true }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
  } catch (error: any) {
    ...
    if (errorMsg.includes('Auth session missing!')) {
      status = 401;
    } else if (errorMsg.includes('users_phone_key') || errorMsg.includes('telefone já está vinculado') || errorMsg.includes('phone')) {
      errorMsg = 'Este número de telefone já está vinculado a outra conta.';
      status = 409;
    } else if (errorMsg.includes('já está em uso') || errorMsg.includes('duplicate key') || errorMsg.includes('already registered')) {
      errorMsg = 'Este e-mail já está em uso por outro perfil.';
      status = 409;
    }
